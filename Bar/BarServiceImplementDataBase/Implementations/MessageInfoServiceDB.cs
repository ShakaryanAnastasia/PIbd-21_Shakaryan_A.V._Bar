...
namespace BarServiceImplementDataBase.Implementations
{
    public class MessageInfoServiceDB : IMessageInfoService
    {
        ...
        public MessageInfoServiceDB(BarDbContext context)
        {
        ...
        public List<MessageInfoViewModel> GetList()
        {
            ...
            .Where(rec => !rec.HabitueId.HasValue)
            .Select(rec => new MessageInfoViewModel
            {
                MessageId = rec.MessageId,
                HabitueName = rec.FromMailAddress,
        ...
        {
            MessageInfo element = context.MessageInfos.Include(rec => rec.Habitue).FirstOrDefault(rec => rec.Id == id);
            if (element != null)
            {
                return new MessageInfoViewModel
                {
                    MessageId = element.MessageId,
                    HabitueName = element.Habitue.HabitueFIO,
...
z])@))(?(\[)(\[(\d{1,3}\.){3}\d{1,3}\])|(([0-9a-z][-\w]*[0-9a-z]*\.)+[a-z0-9]{2,17}))");
            ...
            {
                var habitue = context.Habitues.FirstOrDefault(rec => rec.Mail ==
                mailAddress.Value);
                if (habitue != null)
                {
                    message.HabitueId = habitue.Id;
                }
            }
